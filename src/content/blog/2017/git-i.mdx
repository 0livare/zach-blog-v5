...
When you're in the heat of development though, you often don't want to take the time to make your commits beautiful and detailed, meaning that you have to go back and fix them up later. But how do you do that? Well `rebase -i` of course!

As any developer who cares about their commit history knows, that is a command that gets used a lot. Aside from `commit --amend` or recreating them from scratch, `rebase -i` is the only way in Git to make changes to commits that you have already created.
