...
> This post details the struggle that I originally went through to figure out how to perform the simple task of moving a branch in git.
>
> tldr; `git branch -f`
...
> You can avoid the tricky-confusing-sneaky merge of the `pull` command by adding the `--ff-only` flag. That tells git to only merge if it can do a "fast forward merge", which basically just means moving the branch pointer, which is exactly what we want!
>
> In fact, it's a really good idea tell git to default to this functionality, which you can do by running
>
> ```bash
